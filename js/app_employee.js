			...
			for(var i=0;i<leaves.length;i++){
				...
				for(var j=0;j<leave.date_ranges.length;j++){
					...
					if( leave.info.type=="Vacation"||leave.info.type.toLowerCase().includes('force')||leave.info.type.toLowerCase().includes('mandatory')||leave.info.type=="Sick"||leave.info.type=="Undertime" ){
                        ...
                        $scope.computations.factors.push({type:'Special Priviledge',amount:{v:0,s:0},balance:{v:0,s:0},remarks:'Special Priviledge Leave (WOP)',date:moment(range.start_date,$rootScope.dateFormat),leave_info:leave.info,date_range:range});
                        ...
                        $scope.computations.factors.push({type:'Parental Leave',amount:{v:0,s:0},balance:{v:0,s:0},remarks:'Parental Leave (WOP)',date:moment(range.start_date,$rootScope.dateFormat),leave_info:leave.info,date_range:range});
					}
				...
				for(var j=leave.date_ranges.length-1;j>=0;j--){
					...
					if( leave.info.type=="Vacation"||leave.info.type.toLowerCase().includes('force')||leave.info.type.toLowerCase().includes('mandatory') ){
						...
						if(spLeave<0){
							currV+=spLeave;
							enjoyed.v -= spLeave;
                            $scope.computations.factors.push({type:'Special Priviledge',amount:{v:spLeave,s:0},balance:{s:currS,v:currV},remarks:'Special Priviledge Leave',date:moment(range.start_date,$rootScope.dateFormat),date_range:range,leave_info:leave.info});
							spLeave=0;
						}else{
                            $scope.computations.factors.push({type:'Special Priviledge',amount:{v:0,s:0},balance:{s:currS,v:currV},remarks:'Special Priviledge Leave',date:moment(range.start_date,$rootScope.dateFormat),date_range:range,leave_info:leave.info});
                        }
					}else if(leave.info.type.toLowerCase().includes('parental')){
						//	Parental Leaves	(For Solo Parents)
						pLeave -= creditUsed;
						if(pLeave<0){
							currV+=pLeave;
							enjoyed.v -= pLeave;
                            $scope.computations.factors.push({type:'Parental Leave',amount:{v:pLeave,s:0},balance:{s:currS,v:currV},remarks:'Parental Leave',date:moment(range.start_date,$rootScope.dateFormat),date_range:range,leave_info:leave.info});
							pLeave=0;
						}else{
                            $scope.computations.factors.push({type:'Parental Leave',amount:{v:0,s:0},balance:{s:currS,v:currV},remarks:'Parental Leave',date:moment(range.start_date,$rootScope.dateFormat),date_range:range,leave_info:leave.info});
                        }
					}
					leave.date_ranges.splice(j,1);
				}
